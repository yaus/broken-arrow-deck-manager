...
def cli_verify(service: DeckManagerService) -> int:
    ...
    logger.info("Storage Root: %s (Exists: %s)", service.storage_root, service.storage_root.exists())
    logger.info("Active Decks: %s (Exists: %s)", service.active_decks_path, service.active_decks_path.exists())
    
    game_dir = service.get_game_install_dir()
    logger.info("Game Install: %s", game_dir if game_dir else 'NOT DETECTED')
    ...
    for status in statuses:
        ...
        branch_str = "[SAME BRANCH]" if status.branch_matches else "[DIFF BRANCH]"
        logger.info("  %s %s | %s | %s", match_str, status.name.ljust(20), status.target_branch.ljust(15), branch_str)
...
def main() -> int:
    ...
    parser.add_argument("--cli-verify", action="store_true", help="Print status and exit without GUI")
